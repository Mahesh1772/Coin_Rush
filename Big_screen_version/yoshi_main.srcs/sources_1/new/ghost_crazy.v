module ghost_crazy
    (
        ...
        input wire [9:0] mario_x, mario_y,              // current pixel location of mario
	    input wire [9:0] x, y,                          // current pixel coordinates from vga_sync circuit
	    input wire [25:0] speed_offset,                 // score dependent value that increases ghost's speed
	    output wire [9:0] ghost_crazy_x,ghost_crazy_y,  // output vector for ghost_crazy's x/y position
    ...
    always @(posedge tick, posedge reset)
		...
		if(reset)
			sprite_x_next = 17;          // reset to starting x position
		else if(sprite_x_reg > mario_x)  	// if ghost x pos > mario
		...
		else if(sprite_y_reg > mario_y)  	// if ghost y pos > mario
    ...
    reg [24:0] face_tile_reg;
    wire [24:0] face_tile_next;
    ...
    assign face_tile_next = (face_tile_reg < 25000000)? face_tile_reg + 1 : 0;

    // assign rom index offset, half time between face tiles
    assign face_type = (face_tile_reg < 12500000)? 0 : 16;
    ...
    assign ghost_crazy_x = sprite_x_reg;
    assign ghost_crazy_y = sprite_y_reg;
