module ghost_bottom
    (
	    ...
	    output wire [9:0] ghost_bottom_x, ghost_bottom_y,   // output vector for ghost_top's x/y position
    ...
    always @(posedge tick, posedge reset)
		...
		if(reset)
			sprite_x_next = 620;            	 // reset to starting x position
		else if(mario_y >= 297)         		// if mario is in bottom portion of screen, ghost_B can chase
			begin
		...
		if(reset)
			sprite_y_next = 460;           	// reset to starting y position
		else if(mario_y >= 297)         		// if mario is in bottom portion of screen, ghost_B can chase
			begin
    ...
    assign face_type = (face_tile_reg < 20000000 || mario_y < 297)? 0 : 16;
    ...
    assign ghost_bottom_on = ghost_on && rgb_out != 12'hFFF ? 1 : 0;
	
    // route color_data out
    assign rgb_out = color_data;
	
    // route x/y location out
    assign ghost_bottom_x = sprite_x_reg;
    assign ghost_bottom_y = sprite_y_reg;
